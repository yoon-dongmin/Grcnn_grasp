...
def detect_grasps(q_img, ang_img, width_img=None, no_grasps=1):
    ...
    """
    local_max = peak_local_max(q_img, min_distance=20, threshold_abs=0.2, num_peaks=no_grasps) #가장 quality가 높은 q_img를 가져옴

    grasps = []
    for grasp_point_array in local_max:
        grasp_point = tuple(grasp_point_array) #grasp_point_array를 tuple로 변환 

        grasp_angle = ang_img[grasp_point] #해당 point에 angle값을 가져옴
